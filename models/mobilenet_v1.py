...
class MobileNet(nn.Module):
    def __init__(self, widen_factor=1.0, num_classes=1000, prelu=False):
        ...
        self.upsampling = torch.nn.ConvTranspose2d(int(1024*widen_factor), num_classes, kernel_size=8, stride=8)
...
def mobilenet_1prelu(num_classes):
    """
    Construct MobileNet.
    """
    model = MobileNet(widen_factor=1.0, num_classes=num_classes, prelu=True)
    return model

if __name__=="__main__":
    from torchsummary import summary
    model = DepthWiseBlock(128,128)
    inp = torch.randn(1,128,64,64)
    torch.save(model, "DepthWiseBlock.pth")
    summary(model, inp)