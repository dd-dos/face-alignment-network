...
class FAN(nn.Module):
    ...
    def __init__(self, nStack=4, nModules=1, nHgDepth=4, nFeats=256, nClasses=68, InvRes=False):
        ...
        if InvRes == False:
            ...
            self.conv4 = ConvBlock(self.inplanes*self.expansion, self.nFeats)
        else:
            self.conv2 = InvertedResidual(self.inplanes, self.inplanes*self.expansion, stride=1, expand_ratio=2)
            self.conv3 = InvertedResidual(self.inplanes*self.expansion, self.inplanes*self.expansion, stride=1, expand_ratio=2)
            self.conv4 = InvertedResidual(self.inplanes*self.expansion, self.nFeats, stride=1, expand_ratio=2)

        # Stacking part
        for hg_stack in range(self.nStack):
            self.add_module('m' + str(hg_stack), HourGlass(self.nModules, self.nHgDepth, self.nFeats))

            for conv_module in range(self.nModules):
                if self.InvRes == False:
                    self.add_module('top_m_' + str(hg_stack) + '_' + str(conv_module), ConvBlock(self.nFeats, self.nFeats))
                else:
                    self.add_module('top_m_' + str(hg_stack) + '_' + str(conv_module), InvertedResidual(self.nFeats, self.nFeats, stride=1, expand_ratio=2))


            self.add_module('conv_last' + str(hg_stack),
                            nn.Conv2d(self.nFeats, self.nFeats, kernel_size=1, stride=1, padding=0))
            self.add_module('bn_end' + str(hg_stack), nn.BatchNorm2d(self.nFeats))
            self.add_module('l' + str(hg_stack), nn.Conv2d(self.nFeats,
                                                            self.nClasses, kernel_size=1, stride=1, padding=0))

            if hg_stack < self.nStack - 1:
                self.add_module(
                    'bl' + str(hg_stack), nn.Conv2d(self.nFeats, self.nFeats, kernel_size=1, stride=1, padding=0))
                self.add_module('al' + str(hg_stack), nn.Conv2d(self.nClasses,
                                                                 self.nFeats, kernel_size=1, stride=1, padding=0))
...
class ResNetDepth(nn.Module):
    ...
    def __init__(self, block=Bottleneck, layers=[3, 8, 36, 3], nClasses=68):
        self.inplanes = 64
...
if __name__=="__main__":
    ...
    model = FAN(nStack=1, nModules=2, nHgDepth=2, nFeats=128, nClasses=68, InvRes=True)
    # model = ConvBlock(128,128)
    model.eval()
    torch.save(model, "FAN.pth")
