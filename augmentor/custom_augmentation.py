from augmentor.transformations import color, shadow
...
class OverLayMultipleImage():
    ...
    def augment_image(self, imgs, annos, all_raw_anno, is_scan=False):
        ...
        if not is_scan:
            ...
            if len(resized) == 1:
                p_w = random.uniform(1.05, 1.5)
                p_h = random.uniform(1.05, 1.7)
            else:
                p_w = random.uniform(1.05, 1.1)
                p_h = random.uniform(1.05, 1.1)
            width = int(id_size[1]*p_w)
            height = int(id_size[0]*p_h)
